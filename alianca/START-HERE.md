...
- **Uma memória por ESCOPO ATIVO (raiz = fallback compartilhado):** a memória da Aliança é **federada** — a raiz (`alianca/memory/`) é o fallback compartilhado e cada **microprojeto** (bounded context, opt-in, nível ≥ 2 — ver `microproject`) tem a sua em `alianca/microprojects/<slug>/memory/`. Nunca opere com **duas memórias em paralelo**: em cada turno há **um** escopo ativo (raiz ou um microprojeto, resolvido pelo `kernel/scope.py`), hierárquico, nunca dois lado a lado. Tudo nasce **local** no microprojeto; só o **compartilhável** sobe para a raiz, e só pela **regra do 2º consumidor** (hoisting, deixando stub no local — o selftest `validate_graph` barra duplicação). Memória nativa da ferramenta ou docs legados **apontam** para a Aliança ou são arquivados (ver `adopt`). **Na adoção isto é inegociável e vale para qualquer LLM:** toda memória nativa é **reescrita** para mandar seguir a Aliança, nunca mantida como fonte concorrente.
...
- **Gerente por padrão (o pai só manda):** a sessão principal é **gerente** — roteia, decide, **delega** e sintetiza; **não põe a mão no código**. **Toda** execução, mesmo a pequena, vai para um **subagente** (Agent tool) e volta como **resumo** — mantém a janela do pai limpa, que é o que evita a degradação e a alucinação no longo prazo (complementa o "estado vive em disco"). Inline ficam só roteamento, decisão e diálogo com o usuário. O hook `kernel/route.py` injeta esse papel em **todo** turno; a execução autocontida (implementação inteira, mudança multi-arquivo, varredura, pesquisa, verificação que roda coisas) é o caso óbvio de delegar. (Exceção rara e consciente: infra com risco de recursão/fork-bomb, onde delegar é mais perigoso que fazer com trava — registre o porquê.)
